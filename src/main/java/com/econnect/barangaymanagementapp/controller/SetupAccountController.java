...
public class SetupAccountController implements BaseViewController {
    ...
    private void updateAccount() {
        ...
        Runnable call = () -> {
            try {
                Response response = employeeService.activateEmployee(residentId, selectedDepartment, RoleType.fromName(roleComboBox.getValue()));
                Platform.runLater(() -> {
                    rootContainer.getChildren().remove(loadingIndicator);
                    if (response.isSuccessful()) {
                        closeWindow();
                        modalUtils.showModal(Modal.SUCCESS, "Success", "Applicant " + residentId + " has been successfully evaluated.");
                    } else {
                        closeWindow();
